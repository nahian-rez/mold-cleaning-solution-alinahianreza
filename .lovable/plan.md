...
In the existing `<footer>` (line 408–411), add one small line immediately **above** the existing `Cocoa Dolce · Planning estimate only...` line. It is a plain text link labeled **"View the 90-Day Implementation Roadmap →"** pointing to the same URL, opening in a new tab, styled to match the footer's existing `text-xs text-muted-foreground` tone. No change to the existing `PRICE_BASIS_NOTE` line or the existing planning-estimate line.
